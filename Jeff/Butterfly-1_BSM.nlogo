...
crt turtle.number
  [
    ...
    set shape "butterfly" ; makes the turtle butterfly shaped
...
to-report corridor-width
  ...
  ; set local variable that is the count of patches that were used as true
  let mean-path-dist mean [distance start-patch] of turtles
  ; get the mean distance between turtle location and starting patch
  report(total-patches-used / mean-path-dist) ; return corridor width
end
...
;  plot corridor-width ; send a plot of the current corridor width to plot window
  plot mean-elevation ; send a plot of mean elevation of all turtles
  tick
  if ticks >= 1000 ; once the turtle has moved 1000 ticks it stops
  [
    let final-corridor-width corridor-width
    ; set final corridor width for reporting
    output-print word "Corridor width " (precision final-corridor-width 3)
    ; print corridor width
    output-print word "Final mean elevation " (precision mean-elevation 3)
;    export-plot "Corridor width over time" ( word "Corridor_output_for_q_" q ".csv")
    ; exports the plot as csv file
    stop
  ]
